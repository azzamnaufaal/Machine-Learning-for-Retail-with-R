...
```{r}
library(arules)
data <- read.transactions(file="https://storage.googleapis.com/dqlab-dataset/transaksi_dqlab_retail.tsv", format = "single", sep = "\t", cols = c(1,2), skip = 1)
...
## Output Awal: Statistik 10 Bottom
```{r}
library(arules)
data <- read.transactions(file="https://storage.googleapis.com/dqlab-dataset/transaksi_dqlab_retail.tsv", format = "single", sep = "\t", cols = c(1,2), skip = 1)
...
## Mencari Paket Produk yang bisa dipasangkan dengan Item Slow_Moving
```{r}
library(arules)
transaksi_tabular <- read.transactions(file="https://storage.googleapis.com/dqlab-dataset/transaksi_dqlab_retail.tsv", format = "single", sep = "\t", cols = c(1,2), skip = 1)
...
#Menggabungkan hasil kedua subset
